...
from geesibling.adapters.jax.pipeline.util import OrderedSet, cached_property

def gen_dependency_with_stages(compute_stages, apply_grad_stages):
    ...
    var_layer_id = {}
    for i, sliced_jaxpr in enumerate(itertools.chain(compute_stages,
                                              apply_grad_stages)):
        for var in sliced_jaxpr.jaxpr.invars:
            if var in var_layer_id:
                d[i, var_layer_id[var]] = 1
            else:
                pass
        for var in sliced_jaxpr.jaxpr.outvars:
            var_layer_id[var] = i
    return d
...
class PipelineSchedule(metaclass=ABCMeta):
    ...
    def pprint_schedule(self, to_print=True):
...
#分阶段分时钟周期地执行前向计算和反向传播，最后一个时钟周期表示应用梯度的执行
