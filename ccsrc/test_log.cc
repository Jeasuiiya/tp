...
int main(int, char *[]) {
    ...
    // for later inspection. When needed, call dump_backtrace() to see what
    // happened:
    spdlog::enable_backtrace(
        10);  // create ring buffer with capacity of 10  messages
    for (int i = 0; i < 100; i++) {
...
void rotating_example() {
    ...
    auto rotating_logger = spdlog::rotating_logger_mt(
        "some_logger_name", "logs/rotating.txt", 1048576 * 5, 3);
}
...
void binary_example() {
    ...
    // spdlog::to_hex(buf)); logger->info("hexdump style: {:a}",
    // spdlog::to_hex(buf)); logger->info("hexdump style, 20 chars per line
    // {:a}", spdlog::to_hex(buf, 20));
}
...
// this will add custom flag '%*' which will be bound to a <my_formatter_flag>
// instance
