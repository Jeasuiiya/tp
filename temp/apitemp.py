...
#                init_global_cluster(cluster="ray")
                ...
                for mesh_idx, physical_mesh in enumerate(virtual_mesh.launched_physical_mesh_group):
                    for worker in physical_mesh.workers:
                        abstract_args = map(abstractify_with_aval, args_flat)
                        ray.get(worker.get_stages_to_run.remote(func,f,parallel_method.policy, parallel_method.method, args, kwargs,batch_invars, parallel_method.num_microbatch, args_flat,parallel_method.layer_num, *abstract_args))
                for mesh_idx, physical_mesh in enumerate(virtual_mesh.launched_physical_mesh_group):
                    for worker in physical_mesh.workers:
